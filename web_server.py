...
def apply_routing_hint(message: str, agent_choice: AgentChoice) -> str:
    ...
    Farm Bill/USDA/food security/commodity are nested under agriculture_agent; policing/courts are nested under
    criminal_justice_agent; K-12/higher-ed/student aid/workforce pathways are nested under education_agent;
    climate/conservation/EPA/public-lands are nested under environment_agent; diplomacy/treaties/alliances/regional
    strategy are nested under foreign_relations_agent; healthcare lanes are nested under healthcare_agent; housing lanes
    are nested under housing_agent; immigration lanes are nested under immigration_agent; infrastructure lanes are nested
    under infrastructure_agent; national-security lanes are nested under national_security_agent; technology lanes are
    nested under technology_agent; transportation lanes are nested under transportation_agent; and veterans lanes are
    nested under veterans_agent.
    """
    ...
    if agent_choice == "k12_agent":
        return (
            "[User routing preference: education_agent]\n\n"
            "[Sub-delegate: k12_agent]\n\n"
            + message
        )
    if agent_choice == "higher_education_agent":
        return (
            "[User routing preference: education_agent]\n\n"
            "[Sub-delegate: higher_education_agent]\n\n"
            + message
        )
    if agent_choice == "student_aid_agent":
        return (
            "[User routing preference: education_agent]\n\n"
            "[Sub-delegate: student_aid_agent]\n\n"
            + message
        )
    if agent_choice == "workforce_pathways_agent":
        return (
            "[User routing preference: education_agent]\n\n"
            "[Sub-delegate: workforce_pathways_agent]\n\n"
            + message
        )
    if agent_choice == "climate_clean_energy_agent":
        return (
            "[User routing preference: environment_agent]\n\n"
            "[Sub-delegate: climate_clean_energy_agent]\n\n"
            + message
        )
    if agent_choice == "conservation_agent":
        return (
            "[User routing preference: environment_agent]\n\n"
            "[Sub-delegate: conservation_agent]\n\n"
            + message
        )
    if agent_choice == "epa_policy_agent":
        return (
            "[User routing preference: environment_agent]\n\n"
            "[Sub-delegate: epa_policy_agent]\n\n"
            + message
        )
    if agent_choice == "public_lands_agent":
        return (
            "[User routing preference: environment_agent]\n\n"
            "[Sub-delegate: public_lands_agent]\n\n"
            + message
        )
    if agent_choice == "diplomacy_agent":
        return (
            "[User routing preference: foreign_relations_agent]\n\n"
            "[Sub-delegate: diplomacy_agent]\n\n"
            + message
        )
    if agent_choice == "treaties_agent":
        return (
            "[User routing preference: foreign_relations_agent]\n\n"
            "[Sub-delegate: treaties_agent]\n\n"
            + message
        )
    if agent_choice == "alliances_agent":
        return (
            "[User routing preference: foreign_relations_agent]\n\n"
            "[Sub-delegate: alliances_agent]\n\n"
            + message
        )
    if agent_choice == "regional_strategy_agent":
        return (
            "[User routing preference: foreign_relations_agent]\n\n"
            "[Sub-delegate: regional_strategy_agent]\n\n"
            + message
        )
    if agent_choice == "access_affordability_agent":
        return (
            "[User routing preference: healthcare_agent]\n\n"
            "[Sub-delegate: access_affordability_agent]\n\n"
            + message
        )
    if agent_choice == "medicare_medicaid_agent":
        return (
            "[User routing preference: healthcare_agent]\n\n"
            "[Sub-delegate: medicare_medicaid_agent]\n\n"
            + message
        )
    if agent_choice == "public_health_agent":
        return (
            "[User routing preference: healthcare_agent]\n\n"
            "[Sub-delegate: public_health_agent]\n\n"
            + message
        )
    if agent_choice == "insurance_markets_agent":
        return (
            "[User routing preference: healthcare_agent]\n\n"
            "[Sub-delegate: insurance_markets_agent]\n\n"
            + message
        )
    if agent_choice == "housing_supply_agent":
        return (
            "[User routing preference: housing_agent]\n\n"
            "[Sub-delegate: housing_supply_agent]\n\n"
            + message
        )
    if agent_choice == "housing_affordability_agent":
        return (
            "[User routing preference: housing_agent]\n\n"
            "[Sub-delegate: housing_affordability_agent]\n\n"
            + message
        )
    if agent_choice == "fair_housing_agent":
        return (
            "[User routing preference: housing_agent]\n\n"
            "[Sub-delegate: fair_housing_agent]\n\n"
            + message
        )
    if agent_choice == "rental_markets_agent":
        return (
            "[User routing preference: housing_agent]\n\n"
            "[Sub-delegate: rental_markets_agent]\n\n"
            + message
        )
    if agent_choice == "homelessness_agent":
        return (
            "[User routing preference: housing_agent]\n\n"
            "[Sub-delegate: homelessness_agent]\n\n"
            + message
        )
    if agent_choice == "border_policy_agent":
        return (
            "[User routing preference: immigration_agent]\n\n"
            "[Sub-delegate: border_policy_agent]\n\n"
            + message
        )
    if agent_choice == "legal_immigration_agent":
        return (
            "[User routing preference: immigration_agent]\n\n"
            "[Sub-delegate: legal_immigration_agent]\n\n"
            + message
        )
    if agent_choice == "immigration_pathways_agent":
        return (
            "[User routing preference: immigration_agent]\n\n"
            "[Sub-delegate: immigration_pathways_agent]\n\n"
            + message
        )
    if agent_choice == "humanitarian_processing_agent":
        return (
            "[User routing preference: immigration_agent]\n\n"
            "[Sub-delegate: humanitarian_processing_agent]\n\n"
            + message
        )
    if agent_choice == "water_systems_agent":
        return (
            "[User routing preference: infrastructure_agent]\n\n"
            "[Sub-delegate: water_systems_agent]\n\n"
            + message
        )
    if agent_choice == "grid_resilience_agent":
        return (
            "[User routing preference: infrastructure_agent]\n\n"
            "[Sub-delegate: grid_resilience_agent]\n\n"
            + message
        )
    if agent_choice == "public_works_agent":
        return (
            "[User routing preference: infrastructure_agent]\n\n"
            "[Sub-delegate: public_works_agent]\n\n"
            + message
        )
    if agent_choice == "broadband_agent":
        return (
            "[User routing preference: infrastructure_agent]\n\n"
            "[Sub-delegate: broadband_agent]\n\n"
            + message
        )
    if agent_choice == "defense_strategy_agent":
        return (
            "[User routing preference: national_security_agent]\n\n"
            "[Sub-delegate: defense_strategy_agent]\n\n"
            + message
        )
    if agent_choice == "deterrence_agent":
        return (
            "[User routing preference: national_security_agent]\n\n"
            "[Sub-delegate: deterrence_agent]\n\n"
            + message
        )
    if agent_choice == "homeland_security_agent":
        return (
            "[User routing preference: national_security_agent]\n\n"
            "[Sub-delegate: homeland_security_agent]\n\n"
            + message
        )
    if agent_choice == "military_affairs_agent":
        return (
            "[User routing preference: national_security_agent]\n\n"
            "[Sub-delegate: military_affairs_agent]\n\n"
            + message
        )
    if agent_choice == "ai_innovation_agent":
        return (
            "[User routing preference: technology_agent]\n\n"
            "[Sub-delegate: ai_innovation_agent]\n\n"
            + message
        )
    if agent_choice == "cybersecurity_agent":
        return (
            "[User routing preference: technology_agent]\n\n"
            "[Sub-delegate: cybersecurity_agent]\n\n"
            + message
        )
    if agent_choice == "rd_agent":
        return (
            "[User routing preference: technology_agent]\n\n"
            "[Sub-delegate: rd_agent]\n\n"
            + message
        )
    if agent_choice == "semiconductors_agent":
        return (
            "[User routing preference: technology_agent]\n\n"
            "[Sub-delegate: semiconductors_agent]\n\n"
            + message
        )
    if agent_choice == "digital_competitiveness_agent":
        return (
            "[User routing preference: technology_agent]\n\n"
            "[Sub-delegate: digital_competitiveness_agent]\n\n"
            + message
        )
    if agent_choice == "highways_agent":
        return (
            "[User routing preference: transportation_agent]\n\n"
            "[Sub-delegate: highways_agent]\n\n"
            + message
        )
    if agent_choice == "transit_agent":
        return (
            "[User routing preference: transportation_agent]\n\n"
            "[Sub-delegate: transit_agent]\n\n"
            + message
        )
    if agent_choice == "rail_agent":
        return (
            "[User routing preference: transportation_agent]\n\n"
            "[Sub-delegate: rail_agent]\n\n"
            + message
        )
    if agent_choice == "aviation_agent":
        return (
            "[User routing preference: transportation_agent]\n\n"
            "[Sub-delegate: aviation_agent]\n\n"
            + message
        )
    if agent_choice == "maritime_mobility_agent":
        return (
            "[User routing preference: transportation_agent]\n\n"
            "[Sub-delegate: maritime_mobility_agent]\n\n"
            + message
        )
    if agent_choice == "dot_programs_agent":
        return (
            "[User routing preference: transportation_agent]\n\n"
            "[Sub-delegate: dot_programs_agent]\n\n"
            + message
        )
    if agent_choice == "va_benefits_agent":
        return (
            "[User routing preference: veterans_agent]\n\n"
            "[Sub-delegate: va_benefits_agent]\n\n"
            + message
        )
    if agent_choice == "veteran_education_agent":
        return (
            "[User routing preference: veterans_agent]\n\n"
            "[Sub-delegate: veteran_education_agent]\n\n"
            + message
        )
    if agent_choice == "veteran_employment_agent":
        return (
            "[User routing preference: veterans_agent]\n\n"
            "[Sub-delegate: veteran_employment_agent]\n\n"
            + message
        )
    if agent_choice == "veteran_housing_supports_agent":
        return (
            "[User routing preference: veterans_agent]\n\n"
            "[Sub-delegate: veteran_housing_supports_agent]\n\n"
            + message
        )
    if agent_choice == "military_families_agent":
        return (
            "[User routing preference: veterans_agent]\n\n"
            "[Sub-delegate: military_families_agent]\n\n"
            + message
        )
